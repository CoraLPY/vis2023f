 ...
 let data_array=[]; 
      ...
      const asyncRunFail = async () => {
        let firstRun = await d3.text("../data.csv").then(function (data) {
            var parsedCsv = d3.csvParse(data, (d, i, columns) => {
             d3.autoType(d);
             const objArg = new Object();
               objArg.學號 = d.學號;
               objArg.姓名 = d.姓名;
               objArg.作業一 = d.作業一;
             data_array.push(objArg);
             return d;
           });
      ...
      asyncRunFail().then(string => {
        ...
        //JSON.parse(JSON.stringify(data_array))
        var char = Histogram(data_array, { 
            value: d => d['作業一'],
            label: "作業一(%) →",
            width: 500,
            height: 500,
